...
void update(int i,int x,int start,int end,int treenode)
{
	if(i>end || i<start || start>end)
		return;
	else if(i==start && i==end)
		{
			tree[treenode].maxsum=x;
			tree[treenode].suffix=x;
			tree[treenode].prefix=x;
			tree[treenode].sum=x;
		}
	else
	{
		...
		update(i,x,mid+1,end,2*treenode+1);
		update(i,x,start,mid,2*treenode);
